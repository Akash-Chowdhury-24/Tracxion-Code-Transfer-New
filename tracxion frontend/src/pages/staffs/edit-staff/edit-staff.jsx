...
const fetchStaff = async (staffId) => {
  const response = await fetch(
    `${import.meta.env.VITE_API_BASE_URL}/opronsinglerecord`,
    {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      const [staffData] = await Promise.all([fetchStaff(staffId)]);
      // console.log("staff roles fetched", staffRoles);
      // console.log("staff departments fetched", staffDepartments);
      if (staffData) {
        setStaffFormData({
          firstName: staffData?.staff_first_name,
          lastName: staffData?.staff_last_name,
          email: staffData?.staff_email,
          phone: staffData?.staff_phone,
          role: staffData?.staff_role,
          department: staffData?.staff_department,
          staffImage: staffData?.staff_logo_url,
        });
      }
      setLoading(false);
    }
  ...
  const handleEditStaff = async () => {
    ...
    try {
      ...
      const [response, response2] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_BASE_URL}/uploadstaff`, {
          method: 'POST',
          body: dataToSend,
        }),
      ...
      const data = await response.json();
      console.log("staff edited", data);
      if ((data.status === 200 || data.message === `Hello ${staffFormData.firstName} Your POST request worked`) && (response2.status === 200)) {
        navigate('/staffs');
        return;
      }
