...
const CommonTable = ({
  ...
  // Use external pagination state if server-side, otherwise use internal state
  const currentPage = serverSidePagination ? (externalCurrentPage || 1) : internalCurrentPage;
  const totalPages = serverSidePagination ? (externalTotalPages || 1) : Math.ceil(tableData?.length / itemsPerPage);
  const totalItems = serverSidePagination ? (externalTotalItems || 0) : (tableData?.length || 0);
  ...
  // Reset internal page to 1 when tableData changes (client-side pagination only)
  useEffect(() => {
    if (!serverSidePagination && tableData?.length > 0) {
      setInternalCurrentPage(1);
    }
  }, [tableData, serverSidePagination]);

  // Pagination calculations
  // For server-side pagination, use tableData directly (already paginated)
  ...
  const handlePageChange = (pageNumber) => {
    if (pageNumber >= 1 && pageNumber <= totalPages) {
      if (serverSidePagination) {
        // Call parent's onPageChange callback for server-side pagination
  ...
  const startRange = serverSidePagination 
    ? ((currentPage - 1) * itemsPerPage + 1)
    : (indexOfFirstItem + 1);
  const endRange = serverSidePagination
